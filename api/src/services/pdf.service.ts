...
// Brand colors: #C4664A accent, #FAF2EC bg, #2A1510 text
export async function generateMembershipCertificate(
  ...
  const qrDataUrl = await QRCode.toDataURL(profileUrl, {
    width: 120,
    margin: 1,
    color: { dark: '#2A1510', light: '#FAF2EC' },
  });
  ...
  return new Promise<Buffer>((resolve, reject) => {
    ...
    const cx = W / 2;
    doc.moveTo(cx - 140, 188).lineTo(cx + 140, 188).lineWidth(1).stroke('#C4664A');
    ...
    const cardW = W - 160;
    const cardH = 110;
    doc.roundedRect(cardX, cardY, cardW, cardH, 6).fill('#2A1510');
    ...
    doc.fontSize(8).fillColor('#C4664A');
    ...
    doc.fontSize(13).fillColor('#FAF2EC');
    ...
    doc.roundedRect(badgeX, badgeY, badgeW, 20, 10).fill('#C4664A');
    doc.fontSize(8).fillColor('#FAF2EC')
    ...
    doc.fontSize(7).fillColor('#7A5040')
      .text('Scan to verify', qrX, qrY + qrSize + 4, { width: qrSize, align: 'center' });

    // Chess symbols decoration (left of QR)
    doc.fontSize(20).fillColor('#C4664A')
      .text('\u2654 \u2655 \u2656 \u2657 \u2658 \u2659', 60, qrY + 30, { width: qrX - 80 });

    // Footer line
    const footerY = H - 60;
    doc.moveTo(40, footerY).lineTo(W - 40, footerY).lineWidth(0.5).stroke('#C4664A');

    doc.fontSize(9).fillColor('#7A5040')
      .text(
