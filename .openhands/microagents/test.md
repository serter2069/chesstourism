...
with sync_playwright() as p:
    ...
    # Test each route from pageRegistry
    for route in routes:
        url = f"{STAGING}{route}"
...
```python
# Example scenario: { "name": "Guest views tournaments", "steps": ["open /tournaments", "scroll down", "tap first tournament"] }
...
2. **Mobile viewport only** — 390×844 (iPhone 14 Pro size)
